...
- cowlib 2.19.0 on the library graph (latest Hex release as of 2026-09-07)
- Transitive through cowboy and Phoenix HTTP stacks
- Example mix.lock files may still pin an older cowlib. mix.exs ignore_advisories applies to the library project audit.

## Symptom

CI mix hex.audit fails on Hex 2.5 advisory checks for cowlib. Local mix hex.audit that only checks retired packages can still exit 0.
